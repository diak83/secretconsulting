...
const PREVIEW_DATA = {
  1: (user, saju) => `사주 명리학의 정밀 분석 결과, ${user.name}님은 ${DAY_MASTERS[saju.dayMaster].name}의 기운을 타고났습니다.\n상위 1%가 가진 본능적 직관을 가졌지만, 특정 오행의 불균형으로 인해 현재의 성취에 한계를 느낄 수 있습니다.`,
  2: (user, saju) => {
  ...
  if (menuId === 1) {
    ...
    if (age <= 7) {
      ...
      analysis1Text = [
        ...
        `오히려 당신의 뇌 구조는 ${dm.nature}처럼 외부의 혼재된 정보를 역동적으로 가공하고, 본인만의 거대하고 정교한 논리 체계로 재창조해 내는 압도적인 스케일의 정유 공장에 가깝습니다. 따라서 학원 일타 강사가 일방적으로 칠판에 떠먹여 주는 주입식 강의를 멍하니 쳐다보거나 족집게 요약본을 의미 없이 깜지 쓰듯 외우는 하급의 방식으로는, ${dm.strength}이라는 당신의 타고난 거대한 지적 자산과 잠재력을 절대로 10%조차 발현시킬 수 없습니다.`,
      ...
      prescriptionText = [
        ...
        `【 STEP 2. 명리학적 에너지 생체 사이클에 맞춘 소름 돋는 과목 재배치 전략 】\n수험생의 뇌는 무한정 돌아가는 기계가 아닙니다. 하루 중 당신의 오행 에너지가 가장 충만하고 전두엽이 가장 맑은 골든 타임(주로 수면 직후 이른 아침 6시~9시)에는, 본인이 평소에 가장 회피하고 싶어 하거나 고도의 치밀한 수리력이 요구되는 수학 킬러 문항 세트, 국어 독서 고난도 과학/철학 지문 등 가장 무겁고 뇌를 쥐어짜야 하는 과목을 전략적으로 전면 배치하여 뇌의 최고 에너지를 가장 먼저 가차 없이 쏟아부으십시오. 반대로 체력과 집중력이 바닥을 치며 방전되기 시작하는 늦은 오후나 심야에는 절대로 뇌를 괴롭히는 새로운 고난도 개념을 펴지 마십시오. 이때는 가벼운 사탐/과탐 지엽 암기나 영단어 1초 스캐닝, 어제 만들어둔 오답 노트를 가볍게 훑어보는 식으로 뇌의 피로도 강약을 완벽하게 조절해야 합니다. 졸리고 피곤해 죽겠는데 킬러 문항을 붙잡고 끙끙대는 것은 운기의 흐름을 정면으로 역행하여 뇌를 파괴하는 미련한 자해 행위입니다.`,
      ...
      analysis2Text = [
        ...
        `그러나 성인기 합격과 승진의 목줄을 잔인하게 틀어쥐고 당신을 깊은 패배의 늪으로 끌어내리는 가장 무서운 주범은 바로 '${saju.lacking}' 기운의 결핍으로 인해 발생하는 '행동과 출력의 마비 현상'에 있습니다. 머릿속으로는 이론을 완벽하게 꿰고 있고 계획도 거창하지만 막상 실전 시험장이나 평가를 받는 실무 프로젝트 앞에서 손이 차갑게 굳어버리거나, 쓸데없는 완벽주의의 저주에 빠져 "아직 준비가 덜 됐다"며 시작조차 하지 못합니다. 결국 ${dm.weakness} 성향이 폭발하여 깊은 무기력증의 슬럼프가 주기적으로 찾아오게 됩니다. 머리에 집어넣은 방대한 최고급 지식을 현실 세계의 점수와 돈, 커리어로 치환해 내는 인출 통로가 바로 이 결핍된 오행의 자리에 위치해 있어 꽉 막혀 있기 때문입니다.`
      ];
      prescriptionText = [
        ...
        `【 STEP 2. 인풋(Input)의 극단적 최소화와 실전 아웃풋(Output) 중심의 역발상 학습법 】\n현재 ${name}님에게 가장 완벽하게 핏(Fit)되며 막힌 뇌의 혈을 시원하게 뚫어줄 절대적인 마스터 솔루션은 바로 [ ${lackProp.action} ] 입니다. 1000페이지짜리 기본서나 두꺼운 전공 서적을 첫 페이지부터 정독하며 완벽히 100% 이해하려 드는 강박은 합격을 영원히 미루는 세상에서 가장 멍청한 짓입니다. 이론을 입력(Input)하는 시간은 전체 공부 시간의 20%로 냉혹하고 가혹하게 줄이십시오. 나머지 80%는 이론을 완벽히 모르더라도 무조건 닥치고 기출문제를 풀거나, 실무에 직접 머리를 박고 부딪혀 깨지며 정답을 역추적해 들어가는 아웃풋 중심의 사후 해석(Reverse Engineering) 방식으로 뇌를 혹사시켜야 합니다. 이 가혹한 비율이 맞춰지는 순간, 뇌가 실전 감각을 체화하며 성과 도출 속도가 3배 이상 소름 돋게 단축됩니다.`,
        `【 STEP 3. 성인 생체 리듬과 명리학적 오행 사이클을 동기화한 스케줄링 전략 】\n에너지 드링크를 때려 붓는 밤샘 벼락치기는 성인에게 절대 통하지 않으며 수명과 뇌세포를 갉아먹을 뿐입니다. 하루 중 당신의 에너지가 가장 맑고 충만한 골든 타임(보통 기상 후 출근 전 2시간 이내)에는 가장 머리를 많이 써야 하는 고난도 기출 분석이나 전체 뼈대가 되는 핵심 논리를 파악하는 데 모든 에너지를 융단폭격하듯 쏟아부으십시오. 체력이 완전히 방전되는 퇴근 후나 늦은 밤 시간에는 뇌를 괴롭히는 새로운 고난도 개념을 절대 펴지 마십시오. 이때는 낮에 짬짬이 정리해 둔 얇은 요약본이나 오답 노트를 아주 가볍고 편안하게 눈으로 회독하는 식으로 뇌의 피로 강약을 완벽하게 조절해야만 무서운 번아웃(Burnout)의 늪을 피할 수 있습니다.`,
      ...
      conclusionText = [
        ...
        `이 피 말리는 성인기의 싸움을 관통하며 당신이 앞으로 가장 경계하고 혐오해야 할 멘탈의 적은, 두꺼운 기본서를 펴놓고 푹신한 의자에 오래 앉아 있었다는 행위 자체에 위안을 얻는 얄팍하고 오만한 안일함과 가짜 노동입니다. 오늘 본 연구소에서 뼈를 깎는 심정으로 진단해 드린 이 뼈아픈 '결핍 오행 기반의 실전 출력 루틴'을 매일의 숨 쉬는 습관 속에 독하게 이식하십시오. 분명히 당신은 인생이라는 잔혹한 시험대 위에서 시장의 꼭대기를 여유롭게 노니는 거대한 포식자로 성장할 것입니다. 당신 내면의 웅장한 가치와 직관을 절대적으로 믿으십시오.`
      ];
    }
  } else if (menuId === 3) {
    analysis1Text = [
    ...
    analysis2Text = [
      `현재 ${name}님이 특정 공간에서 주변 사람들이 혀를 내두를 정도의 무시무시하고 극강의 몰입도를 보여줄 때는, 당신의 사주에 내재된 풍부한 '${excessEl}' 기운과, 그 공간이 내뿜는 환경 에너지가 우연히 완벽한 스파크를 일으키며 톱니바퀴처럼 맞아떨어졌을 때 폭발적으로 발현됩니다. 이 긍정적인 풍수의 스위치가 켜지는 순간, 당신은 주변의 소음도, 시간의 흐름도 완벽히 망각한 채 진공 상태의 우주에 홀로 떠 있는 듯한 압도적인 집중력과 뇌의 풀가동 상태를 경험하게 됩니다.`,
      `하지만 당신의 이 무서운 집중력을 순식간에 산산조각 내고, 당신을 깊은 무기력과 스마트폰 중독의 수렁으로 밀어 넣는 보이지 않는 잔혹한 암살자는 바로 사주에 치명적으로 결핍된 '${saju.lacking}' 기운입니다. 내 사주에 텅 비어 찬 바람이 쌩쌩 부는 이 필수적인 기운을 현실의 물리적인 사물(풍수 인테리어 처방)로 꽉 채워 보완해주지 않은 채, 오히려 나와 상극(相剋)이 되어 귀한 기를 빨아먹는 잡동사니들을 주변에 무방비하게 방치하면 어떻게 될까요? 당신의 소중한 에너지가 사방으로 산란되고 파괴되어 결국 ${dm.weakness}의 형태로 멘탈이 완벽하게 붕괴해 버리게 됩니다. 아무리 오늘 밤 굳게 결심해도 내일 책상에만 앉으면 10분을 넘기지 못하고 엎드리거나 스마트폰을 집어 드는 끔찍한 굴레가 바로 이 환경의 저주 때문입니다.`
    ];
    prescriptionText = [
      ...
      `【 STEP 3. 디지털 디톡스(Digital Detox) 박스의 도입과 시야의 강박적 통제 】\n${name}님의 사주 기운은 시각적이고 청각적인 외부의 자잘한 자극에 무의식적으로 에너지를 너무나 쉽게 빼앗기는 취약한 구조를 띠고 있습니다. 스마트폰의 아주 작은 진동이나 짧은 화면 불빛만으로도 뇌가 집중력의 맥락을 뚝 끊어버립니다. 책상 주변에 내용물이 전혀 보이지 않는 무거운 불투명한 '디지털 디톡스 박스'를 하나 마련하십시오. 공부를 시작하는 앵커링 의식을 마친 후, 스마트폰과 태블릿(인강 시청용이 아니라면)을 무조건 전원을 끄거나 비행기 모드로 전환하여 이 박스 안에 가두고 자물쇠를 채우듯 뚜껑을 닫으십시오. 눈에 보이지 않는 것만으로도 뇌가 멀티태스킹의 유혹과 백그라운드 프로세스에 낭비하는 막대한 에너지를 완벽하게 차단하고 오롯이 공부에만 쏟아부을 수 있습니다.`,
      `【 STEP 4. 결전의 날, 기적을 소환하는 포터블(Portable) 부적화의 비밀스러운 힘 】\n피 말리는 모의고사, 내 인생의 궤도를 좌우할 대학 수능, 혹은 피 튀기는 고시나 승진 면접 등 극도의 긴장감과 타인의 흉한 기운이 충돌하는 실전 당일, 어른의 멘탈마저도 압박감에 도미노처럼 붕괴하기 일쑤입니다. 평소 내 방 책상에서 항상 곁에 두며 당신의 에너지를 깊게 교류했던 바로 그 재질의 작은 소품(예: 처방된 재질의 차가운 고급 금속 만년필, 나무 열쇠고리, 작은 천연 원석, 특정 색상의 손수건 등)을 반드시 교복 주머니나 정장 안주머니 속에 나만의 비밀 부적처럼 은밀하게 지니고 시험장에 당당히 입장하십시오. 시험지를 받고 심장이 미친 듯이 뛰며 머리가 하얗게 백지장처럼 변하려는 패닉의 찰나, 주머니 속으로 조용히 손을 넣어 그 익숙한 기운의 질감을 손끝으로 강하게 쥐어보십시오. 그 순간, 당신의 뇌 신경계는 가장 안정적이고 폭발적인 퍼포먼스를 냈던 '내 방 책상 앞의 고요하고 완벽했던 상태'로 즉각적인 디폴트 세팅을 0.1초 만에 변환해버립니다. 이것이 상위 0.1%가 실전에서 결코 떨지 않는 멘탈 컨트롤의 극비 시크릿입니다.`,
    ...
    analysis1Text = [
      ...
      `현재 당신의 시각을 통해 뇌로 유입되는 오행 밸런스의 현주소는 [ ${elementCountsStr}] 이라는 위태로운 상태입니다. 시각 세포를 통해 끊임없이 뇌로 밀려 들어오는 색상 에너지가, 이 치우친 오행의 불길에 기름을 부어 뇌를 과열시켜 태워버리느냐, 아니면 바싹 마른 사막에 내리는 생명수 같은 단비가 되느냐에 따라 매일매일 당신의 학습 컨디션은 천국과 지옥을 아찔하게 오가게 됩니다.`
    ];
    analysis2Text = [
    ...
    prescriptionText = [
      ...
      `【 STEP 2. 상극(相剋) 컬러의 완전한 격리와 무자비한 시각적 디톡스(Detox) 】\n운명의 긍정 컬러를 주변에 정성껏 채워 넣는 것만큼이나 수십, 수백 배 더 시급하고 중요한 작업이 있습니다. 그것은 당신의 사주에 이미 주체할 수 없이 넘쳐나서 기운을 산만하게 만들고 까닭 없는 분노와 짜증, 피로감을 돋우는 '흉(凶)한 색상'들을 내 시야에서 완전히 치워버리고 멸종시키는 것입니다. 예를 들어 눈의 피로를 가중시키는 자극적인 쨍한 형광색 펜이나 포스트잇, 시선을 강제로 빼앗아 뇌를 흥분시키는 강렬한 붉은색 계열의 원색 소품이나 인형, 시각적 멀미를 유발하여 뇌파를 흩뜨리는 어지러운 기하학 패턴의 담요나 쿠션 등은 당장 책상 서랍 안쪽이나 뚜껑이 닫히는 불투명한 박스 안으로 완벽하게 숨기십시오. 시야에 걸리는 색상의 노이즈와 쓰레기 정보들을 무자비하게 제거하는 것만으로도, 산만하게 널뛰던 뇌파가 고요하게 정돈되며 의자에 엉덩이를 붙이고 집중할 수 있는 절대 시간이 즉각적으로 1~2시간 이상 비약적으로 늘어나는 기적을 오늘 당장 체험할 수 있습니다.`,
