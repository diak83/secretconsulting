...
// 🔥 10가지(일간)로 정밀 세분화된 기질 맵핑표 (ImgBB 실제 URL 연결 유지)
...
// 🔥 증발했던 오리지널 10,000자급 VVIP 전문 리포트 100% 원복 엔진 🔥
const generateProfessionalReport = (user: any, saju: any, menuId: number) => {
  ...
  let analysis1Text: string[] = []; let analysis2Text: string[] = []; let prescriptionText: string[] = []; let summaryText = ""; let conclusionText: string[] = [];
...
export default function App() {
  ...
  const fetchSajuFromAPI = async (dateString: string, timeString: string, isTimeUnknown: boolean, calendarType: string) => {
    return new Promise(async (resolve) => {
      if (!(window as any).Lunar) {
        await new Promise((res) => {
          ...
          script.onerror = () => res(false); // 로드 실패 시 무한 대기 방지 가드
      ...
      setTimeout(() => {
        try {
          if (!(window as any).Lunar) throw new Error("Lunar script not loaded");
          ...
          if (calendarType === 'lunar' || calendarType === 'leap') {
            lunarObj = (window as any).Lunar.fromYmdHms(year, month, day, hour, min, 0);
          } else {
            const solarObj = (window as any).Solar.fromYmdHms(year, month, day, hour, min, 0);
          ...
          const elementsCount: Record<string, number> = {'목(나무)':0, '화(불)':0, '토(흙)':0, '금(쇠)':0, '수(물)':0};
          ...
          if (lacking === mainElement) {
            ...
            lacking = sortedElements.find(([el]) => el !== mainElement)?.[0] || '수(물)';
          }
      ...
      <div className="no-print relative z-10">
        ...
        {currentView === 'intro' && (
          <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6 py-12">
            ...
            {/* 🔥 카카오톡 인앱 브라우저 경고 배너 🔥 */}
            <div className="w-full bg-[#E8C87A]/20 border border-[#E8C87A]/50 text-[#E8C87A] text-[11px] p-3 rounded-xl mb-6 text-center leading-relaxed backdrop-blur-sm print:hidden shadow-lg break-keep">
