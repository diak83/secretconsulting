...
const firebaseConfig = {
  ...
  authDomain: "secretconsulting.firebaseapp.com",import React, { useState, useEffect, useRef } from "react";
import {
  BookOpen, Lightbulb, Package, Home as HomeIcon, Briefcase, Star,
  Download, Lock, ChevronLeft, MessageCircle, Building, Crown, Sprout,
  Sun, Mountain, Zap, Droplets, Share2, Copy
...
// 🔥 폭탄 1번 완전 복구: 일간별 고유 선천 본성 딕셔너리 (나무 묘사 돌려막기 대수술 완료) 🔥
...
// 🔥 폭탄 1번 완전 복구: 메뉴별 완전히 격리된 3대 맞춤 솔루션 매트릭스 🔥
...
// 🔥 폭탄 3번 수술 완료: 7개 리포트 본문을 100% 독립된 대치동 정밀 에세이 문단으로 완전 분리 🔥
const generateProfessionalReport = (user: any, saju: any, menuId: number) => {
  ...
  if (menuId === 1) { // 1. 학습 스타일
    title1 = "✨ [VVIP 명식 해단식] 선천 인지 필터의 정밀 해부";
    p1 = `${timePhrase} 정밀 파싱한 결과, ${name}님은 ${DM_NATURE_DESCRIPTIONS[saju.dayMaster] || DM_NATURE_DESCRIPTIONS['甲']}\n\n현재 원국을 지배하고 있는 오행의 분포를 스캔해 보면 [ ${elementCountsStr}] 로 구성되어 있습니다. 사주 명리학에서 특정 기운이 이처럼 쏠리거나 비어버리는 불균형은 지식의 입력(Input)과 출력(Output) 과정에서 치명적인 생체 에너지 병목 현상을 일으킵니다. 특히 활자를 눈으로 읽고 이해하는 것과 그것을 시험장이나 현장에서 오차 없이 끄집어내는 속도 사이에 괴리가 발생하는 근본 원인이 바로 이 선천적 인지 밸런스의 왜곡에 있습니다.`;
    title2 = "⚖️ [운기의 밸런스 분석] 극강의 천재성과 정보 누수 슬럼프의 경계";
    p2 = `현재 ${name}님의 지적 패턴에서 뿜어져 나오는 번뜩이는 통찰력과 극강의 몰입도는 원국 내 가장 풍부한 '${excessEl}' 기운에서 발현됩니다. 이 에너지가 알파파와 공명하는 순간, 남들이 수개월에 걸쳐 이해할 복잡한 개념의 뼈대를 단숨에 관통해 냅니다. 본인이 가치를 느끼는 과제에 한해서는 밤을 새워도 지치지 않는 연산력을 과시합니다.\n\n그러나 지적 성취를 방해하는 핵심 병목은 '${saju.lacking}' 기운의 결핍으로 인한 '정보 저장소의 누수 현상'입니다. 활자를 눈으로 바를 땐 다 아는 것 같지만 뇌 신경망에 단단히 결박되지 않아 실전 연산에서 렉이 걸립니다. 미련하게 엉덩이 힘으로 버티는 양치기가 당신에게 최악의 독약인 이유입니다.`;
    title3 = "🗝️ [VVIP 프라이빗 시크릿 솔루션] 1등급 도약을 위한 3대 행동 프로토콜";
    p3 = (SOLUTION_MATRIX[1] || {})[saju.lacking] || (SOLUTION_MATRIX[1] || {})["수(물)"];
    p4 = `최상위권 0.1%의 압도적인 성취는 미련한 시간 싸움에서 오지 않습니다. 사주 원국에 치명적으로 결핍된 에너지를 치밀하게 주입하는 '예열 워밍업'과 뇌 신경망을 해킹하는 '복습 템플릿'을 타협 없이 지켜낼 때 기적이 일어납니다.`;
    p5 = `결론적으로 ${name}님의 명식 도화지는 기존의 지루하고 낡은 룰을 무참히 짓밟고 본인만의 거대한 왕국을 견고하게 세울 수 있는 폭발적인 포텐셜의 원석입니다. 골방에서 칼을 갈며 고독하게 쌓아 올린 이 지적 파워가 대운의 강력한 흐름과 맞물리는 순간, ${ultimateGoal}이라는 찬란한 결실로 증명될 것입니다.\n\n세상이 규격화해 놓은 얄팍한 평균의 기준에 본인을 억지로 욱여넣으며 자책하는 감정 낭비를 오늘부로 중단하십시오. 본원인 ${dm.name}의 거친 생명력을 믿고 나만의 뼈대를 세우십시오.`;
  } else if (menuId === 2) { // 2. 효과적인 학습 방법 (손굳음 에세이 완전 분리)
    title1 = "✨ [VVIP 명식 해단식] 지식 가공 및 인출(Output) 엔진 해부";
    p1 = `${name}님의 사주 원국에 각인된 '정보 출력 회로'를 최신 인지과학 관점에서 심층 해부합니다. 당신의 지적 자아인 '${dm.name}'은 단순히 남들이 떠먹여 주는 활자를 수동적으로 적재해 두는 창고형 뇌 구조가 결코 아닙니다. 파편화된 외부 지식을 본인만의 예리한 필터로 역동적으로 분해하고 재조합하여 완전히 새로운 솔루션으로 뱉어내는 압도적인 '가공 공장형' 명식입니다.\n\n이러한 명식 그릇을 지닌 분에게 해설지를 토시 하나 틀리지 않고 베껴 적으라고 강요하는 것은, 초고성능 슈퍼컴퓨터에 도스(DOS) 운영체제를 깔아놓고 왜 연산이 느리냐고 윽박지르는 것과 같은 참혹한 자해 행위입니다. 당신의 뇌는 본인이 직접 목차를 설계하고 남에게 뱉어낼 수 있는 수준의 '능동적 출력 장악력'을 확보했을 때 비로소 시냅스가 폭발적으로 확장됩니다.`;
    ...
    p5 = `당신의 가공 엔진은 험난한 실전 현장에서 개념을 라이브로 뱉어낼 때 비로소 그 경이로운 진가를 발휘합니다. 골방에서의 묵묵한 인출 훈련이 영광스러운 합격증으로 당당히 증명될 것입니다.`;
  } else if (menuId === 3) { // 3. 추천 학습 아이템 (데스크 풍수)
    title1 = "✨ [VVIP 명식 해단식] 공간 파동 역학과 뇌 신경망의 공명 해독";
  ...
  } else if (menuId === 4) { // 4. 컬러 조후 (백지복습 멘트 완전 소각)
  ...
  } else if (menuId === 7) { // 🔥 기획 추가: 7번 '필승 대화법' 10,000자급 독립 컨설팅 에세이 🔥
    ...
    // 🔥 제5폭탄 수술 완료: 7번 대화법 STEP 1의 첫마디를 5대 오행별 맞춤 화법으로 전면 다변화 🔥
    let step1Speech = '"오늘 10분만 창문 열고 환기하고 시작할까, 지금 빡 집중해서 끝내고 밤에 푹 쉴까?"';
    if (saju.main === '목(나무)') {
      step1Speech = '"오늘 수학 대단원 목차 먼저 스케치해볼래, 영어 본문 먼저 스케치해볼래?"';
    } else if (saju.main === '화(불)') {
      step1Speech = '"오늘 1번 문제 엄마한테 일타강사처럼 설명해줄래, 화이트보드에 써서 보여줄래?"';
    } else if (saju.main === '토(흙)') {
      step1Speech = '"오늘 저녁 7시 정각에 딱 시작할래, 7시 10분에 시작할래?"';
    } else if (saju.main === '금(쇠)') {
      step1Speech = '"오늘 45분 집중 타이머 켤래, 50분 타이머 켤래?"';
    }

    title3 = "🗝️ [VVIP 프라이빗 시크릿 솔루션] 상대의 뇌를 여는 3대 필승 대화 프로토콜";
    p3 = `【 STEP 1. '통제 언어'를 '선택권 부여 주파수'로 치환하라 】\n\n"이거 공부했어?"라는 추궁성 제어 언어 대신, [ ${step1Speech} ]라는 '가짜 선택권 프레임'을 설계해 하달하십시오. 상대의 전두엽이 '내가 스스로 결정했다'고 착각하는 순간, 편도체의 반항 스위치가 0.1초 만에 영구 소멸됩니다.\n\n【 STEP 2. 3초 침묵의 뇌파 완충(Buffer) 법칙 】\n\n상대가 가시 돋친 말을 뱉을 때 즉각 반박하는 것은 상대의 전투력에 기름을 붓는 행위입니다. 입술을 닫고 상대의 미간을 차분히 응시하며 속으로 천천히 3초를 세는 완충 루틴을 확보하십시오. 이 서늘하고 묵직한 3초의 물리적 침묵이 상대의 과열된 뇌파를 강제로 냉각시키고, 이어지는 부모의 첫마디에 제왕적인 무게감을 각인시킵니다.\n\n【 STEP 3. 절대 상극의 '금기어 영구 봉인술' 】\n\n명식 구조상 [ ${dm.weakness} ]을 들추어내는 타인과의 비교 발언이나 "네가 그렇지 뭐" 식의 존재 부정 단어는 상대의 자아 뼈대를 무너뜨리는 청각적 흉기입니다. 혀끝까지 이 단어가 차오르는 찰나, 입술을 깨물어서라도 완벽하게 삼켜내어 영구 유배 보내셔야만 부모·자식 간의 지적 연결 고리가 보존됩니다.`;
  ...
  // 🔥 뱃지 충돌 완전 세탁: 7번 메뉴일 땐 밑에 새벽딥워크 뱃지 대신 '대화 전용 3종 뱃지' 강제 할당 🔥
  if (menuId === 7) {
...
export default function App() {
  ...
  // 🔥 제2폭탄 수술: 조후/억부 용신 알고리즘(yongshinWeight)이 100% 탑재된 무결점 만세력 엔진 🔥
    ...
    return new Promise(async (resolve) => {
      ...
      setTimeout(() => {
        try {
          ...
          // 🔥 제2폭탄 수술 완료: 생극제화 가중치 매트릭스 (수생목 왜곡 및 목생화 함정 차단) 🔥
  ...
  // 🔥 제4폭탄 수술 완료: 유저가 취소 눌렀을 때의 오탐지 경고창 매너 소각 🔥
      ...
      <div className="no-print relative z-10">
        ...
        {currentView === 'result' && selectedMenu && (
          <div className="relative z-20 min-h-screen min-h-[750px] bg-[#FDFBF7] text-[#1A1530] pb-12 animate-[sup_0.3s_cubic-bezier(0.34,1.56,0.64,1)]">
            ...
            {/* 🔥 제3폭탄 조치: 결과 뷰 진입 시 0.7초간 노출될 VVIP 궤적 해독 시스루 오버레이 🔥 */}
