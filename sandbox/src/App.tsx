...
const generateProfessionalReport = (user: any, saju: any, menuId: number) => {
  ...
  const talkObj = TALK_PRESCRIPTION[lack] || TALK_PRESCRIPTION['수(물)'];
  ...
  if (menuId === 1) { 
    ...
    } else if (safeSaju?.isRelativelyBalanced) {
  ...
  useEffect(() => {
    try {
      ...
      if (pName && pDate) {
        setUserInfo(prev => ({
          ...(prev || {}), 
          name: safeDecode(pName).slice(0, 10), 
          birthDate: pDate,
          birthTime: safeDecode(params.get('t')), 
