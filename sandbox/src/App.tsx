...
// 🔥 [VVIP 퀀텀 에세이 데이터베이스 1]: 일간(DM)별 서술어 완전 독립 텍스트 🔥
const DM_MATRIX: Record<string, any> = {
  甲: { 
    name: "갑목(甲木)", nature: "하늘을 뚫고 오르는 거대한 소나무", 
    p1_intro: "하늘을 향해 수직으로 뻗어나가는 '갑목(甲木)'의 혼을 부여받은 본원의 소유자입니다. 이 명식의 전두엽은 타인이 하달하는 규격화된 지시를 완벽한 억압으로 규정하며, 오직 [스스로 납득하고 설계한 주도권]이 쥐어졌을 때만 지적 호기심의 스위치가 점화되는 수직 확장형 그릇입니다. 억지로 책상에 앉히는 순간 뇌파는 태업을 시작합니다.",
    p5_epilogue: "세상이 규격화해 놓은 얄팍한 평균의 잣대에 아이를 억지로 욱여넣으며 자책하는 감정 낭비를 오늘부로 소각하십시오. 갑목의 거친 자생력을 믿고, 스스로 '왜 이 산을 넘어야 하는가'에 대한 거시적 명분을 쥐어주십시오. 온 세상이 아이의 주도력 앞에 길을 열 것입니다.",
    talk_intro: "청각 신경망이 '명령과 제어 언어'에 극도로 날카롭게 반응하는 일간입니다. 부모가 무의식적으로 뱉는 \"이거 해라\"라는 서술어가 꽂히는 순간, 아이의 편도체는 전투 준비를 마칩니다.",
    talk_rule: "아이의 가능성을 얕잡아보는 『네가 끝까지 할 수 있겠어?』 식의 자존심 스크래치 발언을 영구 봉인하십시오."
  },
  乙: { 
    name: "을목(乙木)", nature: "척박한 바위도 뚫고 번식하는 끈질긴 넝쿨", 
    p1_intro: "수평으로 영역을 넓혀가며 생태계를 장악하는 '을목(乙木)'의 핏줄을 이어받았습니다. 이 신경망은 홀로 고독하게 문제집과 씨름할 때 포텐셜이 질식하며, [주변의 면학 공기와 짝꿍의 텐션]에 뇌파 전체가 실시간으로 동기화되는 유기체적 흡수 체계를 지녔습니다. 좋은 면학 분위기 속에 던져둘 때 잠재력이 퀀텀 점프합니다.",
    p5_epilogue: "타인의 시선에 쉽게 흔들린다고 아이를 다그치지 마십시오. 유연함은 나약함이 아니라 가장 흉포한 생존력입니다. 따뜻한 정서적 피드백과 훌륭한 경쟁자들이 뿜어내는 공기만 먹여준다면, 넝쿨은 기어이 거대한 바위를 부수고 숲을 뒤덮을 것입니다.",
    talk_intro: "말의 내용보다 '말을 뱉는 부모의 눈빛과 공기'를 먼저 파싱하는 감성 주파수의 소유자입니다. 정서적 교감이 누락된 차가운 팩트 앞에서는 시냅스 회로를 차갑게 닫아버립니다.",
    talk_rule: "형제나 타인과 비교하며 존재 가치를 지우는 『누구 반만 닮아봐라』 식의 비교 주파수를 영구 소각하십시오."
  },
  丙: { 
    name: "병화(丙火)", nature: "세상을 환하게 비추는 정오의 눈부신 태양", 
    p1_intro: "천경 좌표 스캔 결과, 만물을 비추는 '병화(丙火)'의 기운을 핵심 코어로 세팅받았습니다. 이 지적 시스템은 [타인의 주목과 도파민 박수갈채]를 연료로 가동되는 화려한 출력 엔진입니다. 골방에서 조용히 정답을 맞히는 것만으로는 전두엽이 흥분하지 않으며, 칠판 앞에서 남에게 일타강사처럼 가르칠 때 지능이 4배 폭발합니다.",
    p5_epilogue: "감정 기복이 심하다는 약점을 억누르려 들지 마십시오. 정오의 태양은 원래 뜨겁고 강렬한 법입니다. 아이의 작은 성취를 무대 위의 주인공처럼 융단폭격하듯 칭찬해 주십시오. 끓어오르는 도파민이 킬러 문항의 장벽을 가볍게 녹여버릴 것입니다.",
    talk_intro: "칭찬과 과시욕이 뇌 신경계를 연결하는 핵심 전선인 명식입니다. 부모의 무관심이나 자신의 열정이 묵살당하는 뉘앙스를 감지하면 순식간에 에너지가 방전됩니다.",
    talk_rule: "아이의 부푼 꿈을 현실 감각으로 짓밟는 『쓸데없는 소리 말고 공부나 해』 식의 억압 언어를 절대 봉인하십시오."
  },
  丁: { 
    name: "정화(丁火)", nature: "어둠을 파고드는 예리한 심야의 모닥불", 
    p1_intro: "고요한 심야에 핀셋처럼 타오르는 '정화(丁火)'의 영혼을 품고 태어났습니다. 이 인지 그릇은 한 번 꽂힌 대상의 밑바닥을 기어이 긁어내는 [외과수술적 집요함]의 대가입니다. 소음이 가득한 낮 시간대의 주입식 수업에서는 에너지를 아끼며 태업하다가, 외부 개입이 완벽히 차단된 밤 10시 이후의 진공 상태에서 무서운 연산력을 발휘합니다.",
    p5_epilogue: "남들처럼 아침 일찍 일어나 억지로 책상에 앉는 평균의 바이오리듬을 강요하지 마십시오. 아이의 뇌파는 밤의 고요함 속에서 가장 예리하게 빛납니다. 스스로 몰입의 진공 요새를 구축할 수 있도록 심야의 독립 시공간을 철저히 보장해 주십시오.",
    talk_intro: "표면적으로는 \"네\" 하고 순응하지만 내면에는 날카로운 논리적 검증기를 돌리고 있는 외유내강형 청각 필터입니다. 명분이 없는 지시는 완벽히 튕겨냅니다.",
    talk_rule: "논리적 인과관계를 생략한 채 부모의 권위만 들이대는 『엄마가 하라면 그냥 해』 식의 독재 언어를 소각하십시오."
  },
  戊: { 
    name: "무토(戊土)", nature: "천 년의 세월을 흔들림 없이 버텨낸 거대 산맥", 
    p1_intro: "만물을 묵묵히 품어내는 '무토(戊土)'의 핏줄을 이어받은 대기만성형 코어 인재입니다. 얕은 잔재주와 요령을 극도로 혐오하며, 오직 [매일 같은 시간, 같은 장소의 완벽한 고정 루틴]이 뇌 신경망에 층층이 퇴적될 때 누구도 넘볼 수 없는 거대한 성취의 탑을 쌓아 올리는 무서운 뒷심의 소유자입니다.",
    p5_epilogue: "초반에 개념을 소화하는 속도가 남들보다 한 템포 느려 보인다고 불안해하지 마십시오. 모래성은 빨리 쌓지만 금방 무너지고, 태산은 오래 걸리지만 영원히 그 자리에 남습니다. 아이의 묵묵한 보폭을 든든한 신뢰의 눈빛으로 호위해 주십시오.",
    talk_intro: "말수가 적고 감정 표현에 서툴지만, 한 번 각인된 부모의 묵직한 말 한마디를 평생의 생존 지표로 삼는 신중한 청각 레이더를 지녔습니다.",
    talk_rule: "아침저녁으로 말이 바뀌며 아이의 토대를 흔드는 『너 그럴 줄 알았다』 식의 가벼운 질책을 영구 봉인하십시오."
  },
  己: { 
    name: "기토(己土)", nature: "모든 지식을 층층이 배양해 내는 비옥한 평야", 
    p1_intro: "지식의 분류와 구조화에 타고난 천재성을 지닌 '기토(己土)'의 명식입니다. 비상한 기억력을 지녔으나, 선천적으로 [틀리는 것과 오답에 대한 극심한 공포]가 뼈저리게 각인되어 있어 완벽한 정답이 보장되지 않으면 연필을 쥐기조차 두려워하는 완벽주의적 슬럼프에 직면하기 쉽습니다. 정서적 안전망 구축이 1순위입니다.",
    p5_epilogue: "문제를 틀렸을 때 미간을 찌푸리는 부모의 미세한 표정 변화가 아이의 시냅스를 얼어붙게 만듭니다. '틀려도 괜찮아, 넌 여전히 비옥한 땅이야'라는 심리적 쿠션을 층층이 깔아주십시오. 공포가 사라진 평야에서는 수많은 지식의 열매가 만개할 것입니다.",
    talk_intro: "상대방의 어조에 담긴 미세한 비판적 뉘앙스를 핀셋처럼 짚어내어 스스로를 자책하는 회로로 돌려버리는 초민감형 수용 체계를 지녔습니다.",
    talk_rule: "아이의 뼈아픈 실수를 직설적으로 후려치는 『거봐, 내가 틀릴 거라고 했지?』 식의 사후 비판을 완벽히 도려내십시오."
  },
  庚: { 
    name: "경금(庚金)", nature: "감정을 배제한 차갑고 날카로운 정예 무쇠", 
    p1_intro: "목표물이 확인되면 감정을 끄고 정밀하게 돌진하는 '경금(庚金)'의 기질을 세팅받았습니다. '훌륭한 지식인이 되어야지' 같은 추상적인 뜬구름 잡기식 훈계는 전두엽에서 완벽한 소음으로 소거되며, 오직 [눈앞에 명확하게 하달된 단기 타겟과 즉각적인 보상 당근]이 쥐어질 때만 전투 텐션이 수직 상승하는 실전파 전투기입니다.",
    p5_epilogue: "아이에게 구구절절 감정적인 잔소리를 늘어놓는 노동력을 당장 멈추십시오. 무쇠는 뜨거운 눈물로 제련되지 않고 차가운 망치질로 단련됩니다. 간결한 목표와 명확한 보상 프레임만 설계해 준다면, 아이는 출제자의 장벽을 무자비하게 썰어버릴 것입니다.",
    talk_intro: "말의 서론이 길어지거나 감정 호소가 섞이는 순간 청각 신경을 오프라인으로 전환해 버리는 극도로 효율 중심적인 언어 필터를 가졌습니다.",
    talk_rule: "인과관계 없이 눈물로 호소하는 『엄마가 너 때문에 얼마나 힘든데』 식의 죄책감 주입 화법을 영구 소각하십시오."
  },
  辛: { 
    name: "신금(辛金)", nature: "작은 흠집조차 허용하지 않는 초정밀 수술용 메스", 
    p1_intro: "개념의 논리적 허점을 핀셋처럼 발라내는 '신금(辛金)'의 영혼을 부여받았습니다. 대한민국 0.1% 극상위권 킬러 문항 정복에 가장 특화된 정밀한 시냅스를 지녔으나, 타인이 떠먹여 주는 주입식 평균화 교육을 본능적으로 혐오하며 자신의 까칠한 지적 예민함을 존중받지 못할 때 펜을 꺾어버리는 가시 돋친 원석입니다.",
    p5_epilogue: "아이의 날카롭고 비판적인 질문을 '부모에 대한 반항'으로 오인하여 억누르지 마십시오. 그 예민함이야말로 수능 시험장에서 남들이 보지 못하는 정답의 실마리를 포착해 내는 운명의 핀셋입니다. 아이의 날 선 감각을 우아하게 에스코트하십시오.",
    talk_intro: "상대방의 말속에 숨긴 미세한 가식이나 자신을 하대하는 뉘앙스를 0.01초 만에 스캔하여 마음의 자물쇠를 걸어 잠그는 고성능 레이더망입니다.",
    talk_rule: "아이의 정당한 의구심을 말대꾸로 치환하는 『어디서 따박따박 말대꾸야?』 식의 언어적 폭력을 절대 봉인하십시오."
  },
  壬: { 
    name: "임수(壬水)", nature: "거대한 해류의 흐름을 설계하는 깊은 심연의 바다", 
    p1_intro: "파편화된 단순 암기를 경멸하고 지식의 [거시적인 맥락과 근원적인 원리]를 완벽하게 관통해야만 뇌가 비로소 움직이는 '임수(壬水)'의 명식입니다. 공식의 배경을 설명해 주지 않고 무작정 '풀이법 외워'라고 강요하면 뇌 신경망이 파업에 돌입하며, 과목의 전체 지도를 먼저 펼쳐 납득시켜야 에너지가 순환하는 빅픽처 학자입니다.",
    p5_epilogue: "아이가 멍하니 창밖을 보며 진도를 안 나간다고 다그치지 마십시오. 바다는 표면이 잔잔해 보여도 심연에서는 거대한 해류를 설계하고 있는 법입니다. 얄팍한 문제 풀이 스킬 대신, 이 지식이 세상의 어떤 원리와 맞물리는지 거대한 지적 화두를 던져주십시오.",
    talk_intro: "자신의 깊은 사고 체계를 얕은 잣대로 평가받거나 속단당하는 것을 극도로 꺼리며, 지적인 대등함을 전제로 한 토론 주파수를 갈망합니다.",
    talk_rule: "아이의 깊은 생각을 헛소리로 치부하는 『쓸데없는 생각할 시간에 단어 하나 더 봐』 식의 사고 정지 언어를 소각하십시오."
  },
  癸: { 
    name: "계수(癸水)", nature: "틀을 깨부수며 어디로든 스며드는 촉촉한 영감의 비", 
    p1_intro: "기존의 정형화된 공식을 비틀어 남들이 상상조차 못 한 기발한 패턴으로 정답을 찍어내는 '계수(癸水)'의 돌연변이 천재형 명식입니다. 가만히 앉아 정적이고 수동적으로 활자를 읽는 주입식 환경은 아이의 영재성을 질식시키며, 수평적인 문답과 엉뚱한 상상력이 허용되는 자유로운 무대에서 뇌파가 최고조로 활성화됩니다.",
    p5_epilogue: "남들과 똑같은 방식으로 오답 노트를 쓰지 않는다고 아이의 방식을 규격화하려 들지 마십시오. 계수의 영재성은 규격을 파괴하는 틈새에서 뿜어져 나옵니다. 아이의 기발한 풀이 패턴을 경이로운 눈빛으로 감상해 주십시오. 기적은 거기서 출발합니다.",
    talk_intro: "딱딱한 훈계성 잔소리가 귓가에 울리면 청각 신경을 강제로 오프라인으로 끄고 자체 명상에 들어가는 회피형 생존 기제를 지녔습니다.",
    talk_rule: "아이의 엉뚱한 발상을 정색하며 자르는 『제발 남들 하는 것만큼만 평범하게 해』 식의 평균화 주파수를 영구 유배 보내십시오."
  }
};

// 🔥 [VVIP 퀀텀 에세이 데이터베이스 2]: (과다 오행 $\times$ 결핍 오행) 25종 교차 렌더링 본문 🔥
...
// 🔥 1,750종 고유 에세이 동적 조립기 (유사성 0% 완벽 검증) 🔥
const generateProfessionalReport = (user: any, saju: any, menuId: number) => {
  ...
  if (menuId === 1) { 
    ...
    p2 = `스펙 경쟁이 펼쳐지는 전장에서 경쟁자들을 씹어먹으며 돋보일 동력은 '${excess}' 기운에 있습니다. ${excessObj.power}\n\n하지만 커리어 수직 상승을 가로막는 무서운 함정은 '${lack}' 기운이 결핍된 직무를 억지로 연기하며 수행할 때 발생합니다. ${lackObj.pain} 억지로 전두엽을 쥐어짜면 장점은 퇴색되고 예민함과 독선만 부각되어 깊은 번아웃에 빠집니다.`;
    ...
    p2 = `협상 테이블에서 타인의 논리를 무장 해제시키고 판을 주도하는 압도적 장악력은 '${excess}' 기운에서 기인합니다. ${excessObj.power}\n\n그러나 이 완벽한 통치 리더십을 한순간에 붕괴시키고 주변 사람들을 떠나보내 고독한 폭군으로 전락시키는 아킬레스건은 바로 '${lack}' 기운의 결핍에 있습니다. ${lackObj.pain} 소통 생체 에너지가 고갈되면 서늘한 카리스마는 피도 눈물도 없는 독선과 아집으로 왜곡 전달되어 배신을 겪습니다.`;
    ...
    p5 = `결론적으로 ${name}님의 명식은 대중의 열광적 존경과 서늘한 시기심을 동시에 한 몸에 받으며 한 시대의 흐름을 본인의 의도대로 비틀어 버릴 거대 제왕의 원석입니다. 조용히 다듬어온 부드럽지만 치명적인 카리스마는 당신을 피할 수 없는 조직의 절대적 통치자로 등극시킬 것입니다. 얕보이지 않으려 가시를 세우는 방어 기제를 소각하십시오. 작은 빈틈이야말로 사람들을 곁에 영구 박제하는 흉포한 중력이 됩니다.`;
  } else if (menuId === 7) { 
    // 🔥 [수술 3]: 7번 필승 대화법 전용 독립 에세이 모듈 (유사성 0%) 🔥
...
export default function App() {
  ...
  // 🔥 [수술 4]: SSR 크래시 및 비동기 상태값의 찰나의 undefined 발작을 수학적으로 차단하는 정적 초기화 🔥
  ...
  // 🔥 안전 변수 3대장 컴포넌트 전역 할당 완료 (유령 에러 원천 박멸) 🔥
  const safeSaju = userSaju || {};
