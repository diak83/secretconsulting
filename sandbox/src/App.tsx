...
// 🔥 빌드 파괴 주범이었던 내부 따옴표를 100% 안전한 코너 브라켓(「」)으로 완전 치환한 솔루션 매트릭스 🔥
const SOLUTION_MATRIX: Record<number, Record<string, string>> = {
  1: {
    "목(나무)": "【 STEP 1. 목차 수기 뼈대 각인 】 본문을 읽기 전 반드시 텅 빈 백지에 대단원 목차를 직접 손으로 써서 뇌 속에 지식의 서랍장부터 구획하십시오.\n\n【 STEP 2. 타이머 기반 압축 복습 】 15분 단위의 마이크로 인출 타이머를 걸어 텐션을 유지하십시오.",
    ...
    "금(쇠)": "【 STEP 1. 감정 배제 3분 오답 복기 】 시험을 망쳤을 때의 자책을 소각하고, 어느 지점의 논리 연산이 누락되었나만 핀셋으로 도려내 박제하십시오.\n\n【 STEP 2. 초정밀 압축 치트시트 작성 】 교재 전체를 A4 용지 단 한 장의 수식으로 압축하는 정밀 세공 복습을 가동하십시오.",
    "수(물)": "【 STEP 1. 심야 진공 알파파 딥워크 】 외부 노이즈가 소거된 밤 10시 이후의 고요한 20분을 이 아이만의 킬러 개념 각인 시간으로 사수하십시오.\n\n【 STEP 2. 역발상 마인드맵 복습 】 정답에서 출발해 질문으로 거꾸로 올라가는 역추적 복습 루틴을 이식하십시오."
  },
  2: {
  ...
  3: {
    ...
    "수(물)": "【 STEP 1. 액티브 노이즈 캔슬링 요새 】 주변 소음을 100% 소거해 주는 헤드폰을 책상 위 명당에 고정 배치해 딥워크 진입 스위치로 삼으십시오.\n\n【 STEP 2. 초음파 가습 쿨링 】 메마른 뇌 신경계를 촉촉하게 적셔줄 미세 초음파 가습기를 가동하여 조후 밸런스를 쾌적하게 마감하십시오."
  },
  4: {
  ...
  5: {
    ...
    "화(불)": "【 STEP 1. 도파민 폭발 미디어/스피커 직무 】 본인의 열정을 대중 앞에 출력할 때 쾌감을 느끼는 미디어 디렉터, 일타 강사, 브랜드 크리에이티브 직군을 선점하십시오.\n\n【 STEP 2. 약점인 뒷심 부족 보완 루틴 】 화려한 스타트 이후 마무리가 흐지부지되는 단점을 막기 위해, 프로젝트 마감 자동화 툴을 뼛속까지 이식하십시오.",
  ...
  7: {
    ...
    "화(불)": "【 STEP 1. 인정 주파수 선(先)력 프로토콜 】 대화의 첫마디를 무조건 아이의 작은 성취를 띄워주는 도파민 언어로 시작해 전두엽의 방어 빗장을 0.1초 만에 해제하십시오.\n\n【 STEP 2. 감정 과열 시 타임아웃 쿨링 밸브 】 대화 텐션이 끓어오를 땐 즉시 「엄마가 물 한 잔 마시고 5분 뒤에 다시 이야기할게」라며 스레드를 소각하십시오.",
...
const PREVIEW_DATA: Record<number, any> = {
  ...
  2: (user: any, saju: any) => `현재 ${user.name}님에게 남들과 똑같은 암기식 인강을 강요하는 것은 호랑이를 종이컵에 가두는 자해 행위입니다.\n원국 구조상 지식을 완벽히 내 것으로 박제하기 위해서는 반드시 [입력 30% : 출력 70%]의 외과수술적 인출 회로가 가동되어야 합니다. 당신의 뇌 구조에 최적화된 '골든타임 과목 배치술'은 바로...`,
  3: (user: any, saju: any) => `사주 명리학의 '물상대체론' 관점에서 방의 풍수 파동이 꼬여있으면 아무리 의지력이 강해도 능률이 바닥으로 추락합니다.\n${user.name}님의 사주 원국에 얼어붙은 기운을 순식간에 녹여내고 고요한 알파파 집중 모드를 가동할 책상 위 명당 소품은 바로...`,
  4: (user: any, saju: any) => `시신경을 통해 흡수되는 색채의 파장은 사주의 조후(온도와 습도 밸런스)를 결정짓는 생존 주파수입니다.\n책상 앞에서 ${user.name}님이 유독 극심한 피로감과 번아웃을 겪는 이유는 상극 컬러 독소 때문이며, 시야의 30%를 장악해 전두엽을 식혀줄 운명의 치유 컬러 계열은...`,
  5: (user: any, saju: any) => `냉혹한 경쟁 자본주의 시장에서 ${user.name}님이 남들을 완벽히 압도할 수 있는 선천적 생존 무기는 따로 있습니다.\n평범한 톱니바퀴 부품으로 버려지지 않고 시장 전체의 룰을 뒤흔들며 독보적인 몸값을 쟁취할 수 있는 대체불가 전문 직군은...`,
  6: (user: any, saju: any) => `군중 속에 고요히 섞여 있어도 ${user.name}님은 본능적으로 타인에게 거부할 수 없는 지배력과 신뢰를 뿜어내는 권력 서열 주파수가 존재합니다.\n독선적인 폭군으로 붕괴되지 않고 사람들의 마음을 완벽하게 무장 해제시켜 평생 내 편으로 묶어둘 제왕적 소프트파워의 핵심은...`,
                ...
                return (
                  <div 
                    ...
                    className={`bg-[#111625] border border-[#D4A843]/30 rounded-2xl p-5 text-left cursor-pointer transition-all duration-300 hover:border-[#D4A843] shadow-lg relative overflow-hidden group ${
                      isSeventh ? "col-span-2 bg-gradient-to-r from-[#1A1530] to-[#241A40] border-2 border-[#FFCBA4]" : "col-span-2"
                    }`}
                  >
