...
// 🔥 [백지화 원천 차단 쉴드 1]: 모든 텍스트에 optional chaining(?.)과 fallback(|| '') 강제 삽입 🔥
const PREVIEW_DATA: Record<number, any> = {
  1: (user: any, saju: any) => `명리학적 선천 황경 좌표 스캔 결과, ${user?.name || '고객'}님은 만물을 뚫고 오르는 [${saju?.dayMaster || '甲'}·${(DAY_MASTERS[saju?.dayMaster || '甲']||{}).name || ''}]의 지적 자아를 세팅받았습니다.\n정해진 룰을 강요받을 때 전두엽이 굳어버리며, 원국 내 '${saju?.lacking || '수(물)'}' 기운의 심각한 결핍으로 인해 인풋 대비 아웃풋 병목을 겪고 있습니다. 이 병목을 단 15분 만에 뚫어낼 선천 맞춤형 '예열 스위치'의 정체는 바로...`,
  2: (user: any) => `현재 ${user?.name || '고객'}님에게 남들과 똑같은 암기식 인강을 강요하는 것은 호랑이를 종이컵에 가두는 자해 행위입니다.\n원국 구조상 지식을 완벽히 내 것으로 박제하기 위해서는 반드시 [입력 30% : 출력 70%]의 외과수술적 인출 훈련 회로가 가동되어야 합니다. 당신의 뇌 회로에 최적화된 '골든타임 과목 배치술'은 바로...`,
  3: (user: any) => `사주 명리학의 '물상대체론' 관점에서 방의 풍수 파동이 꼬여있으면 아무리 의지력이 강해도 능률이 바닥으로 추락합니다.\n${user?.name || '고객'}님의 사주 원국에 얼어붙은 기운을 순식간에 녹여내고 고요한 알파파 집중 모드를 가동할 책상 위 명당 소품은 바로...`,
  4: (user: any) => `시신경을 통해 흡수되는 색채의 파장은 사주의 조후(온도와 습도 밸런스)를 결정짓는 생존 주파수입니다.\n${user?.name || '고객'}님이 책상 앞에서 극심한 피로감과 번아웃을 겪는 이유는 상극 컬러 독소 때문이며, 시야의 30%를 장악해 전두엽을 식혀줄 운명의 치유 컬러 계열은...`,
  5: (user: any) => `냉혹한 경쟁 자본주의 시장에서 ${user?.name || '고객'}님이 남들을 완벽히 압도할 수 있는 선천적 생존 무기는 따로 있습니다.\n평범한 톱니바퀴 부품으로 버려지지 않고 시장 전체의 룰을 뒤흔들며 독보적인 몸값을 쟁취할 수 있는 대체불가 전문 직군은...`,
  6: (user: any) => `군중 속에 고요히 섞여 있어도 ${user?.name || '고객'}님에게는 타인에게 거부할 수 없는 지배력과 신뢰를 뿜어내는 선천적 권력 서열 주파수가 존재합니다.\n독선적인 폭군으로 붕괴되지 않고 사람들의 마음을 완벽하게 무장 해제시켜 평생 내 편으로 묶어둘 제왕적 소프트파워의 핵심은...`,
  7: (user: any, saju: any) => `명리학적으로 일간 '${saju?.dayMaster || '甲'}'을 지닌 ${user?.name || '고객'}님의 뇌는 상대방이 뱉는 '특정 단어 파장'에 따라 전두엽이 열리거나 완벽하게 닫히는 양극단의 수용성을 보입니다.\n상대방의 반항심을 0.1초 만에 무장 해제시키고 스스로 책상에 앉게 만들 부모의 '결정적 첫 마디'의 정체는 바로...`,
};
...
const generateProfessionalReport = (user: any, saju: any, menuId: number) => {
  ...
  const dmObj = DM_MATRIX[saju?.dayMaster || '甲'] || DM_MATRIX['甲'];
  const lackObj = LACK_MATRIX[saju?.lacking || '수(물)'] || LACK_MATRIX['수(물)'];
  const excessObj = EXCESS_MATRIX[saju?.excessive || saju?.main || '목(나무)'] || EXCESS_MATRIX['목(나무)'];
  const talkObj = TALK_PRESCRIPTION[saju?.lacking || '수(물)'] || TALK_PRESCRIPTION['수(물)'];
  ...
  const timePhrase = user?.isTimeUnknown ? "태어난 시간의 제약을 초월한 선천 황경 좌표를" : `태어난 시인 [ ${user?.birthTime || '12:00'} ]의 우주적 에너지를`;
  ...
  let symbolsToUse = ELEMENT_PRESCRIPTION[saju?.lacking || '수(물)']?.symbols || [];
  ...
  if (menuId === 1) { 
    ...
    p2 = `현재 ${name}님의 지적 패턴에서 뿜어져 나오는 번뜩이는 통찰력과 극강의 몰입도는 원국 내 가장 풍부한 '${saju?.excessive || '목(나무)'}' 기운에서 발현됩니다. ${excessObj.power || ''}의 천재성을 보입니다.\n\n그러나 성과를 늪으로 끌어내리는 가장 뼈아픈 아킬레스건은 바로 '${saju?.lacking || '수(물)'}' 기운의 완전한 결핍입니다. ${lackObj.pain || ''} 평소 인강이나 교재를 볼 때는 완벽하게 다 아는 것 같지만, 막상 고도의 압박감이 지배하는 ${battleGround}에 들어서는 순간 머릿속이 하얗게 굳어버리는 '블랙아웃(Black-out)' 현상을 겪게 됩니다.`;
    ...
    p2 = `현재 학습이나 업무 과정에서 가끔씩 뿜어져 나오는 소름 돋는 문제 해결 능력은 원국 내에 자리 잡은 풍부한 '${saju?.excessive || '목(나무)'}' 오행 기운 덕분입니다. ${excessObj.power || ''}의 직관 스위치가 점화되는 순간, 남들이 공식에 대입해 가며 한 세월을 보낼 때 당신은 이미 정답의 종착역에 도달해 있는 경이로운 연산 속도를 보여줍니다.\n\n그러나 결정적인 성과 도출의 순간마다 발목을 강하게 부여잡는 주범은 바로 '${saju?.lacking || '수(물)'}' 기운의 결핍으로 인한 '출력 회로의 일시적 마비 현상'입니다. ${lackObj.pain || ''} 이는 입력된 정보를 바깥으로 매끄럽게 인출(Output)해 내는 명리학적 '식상(食傷)' 또는 '관성(官星)'의 유체 밸런스가 꽉 막혀있기 때문입니다.`;
    ...
    p2 = `최상의 컨디션에서 뿜어져 나오는 경이로운 집중력과 슈퍼컴퓨터 같은 연산 속도는 당신의 사주 원국과 공간의 풍수 에너지가 완벽한 공명(Resonance) 궤도에 진입했을 때 발현됩니다. 이 스파크가 튀는 순간, 전두엽 주변의 노이즈가 완벽하게 진공 청소되며 수 시간 동안 미동조차 하지 않는 극강의 알파파 딥워크 상태를 경험하게 됩니다.\n\n그러나 성적과 업무 효율을 늪으로 끌어내리는 방해꾼은 바로 사주 원국에 텅 비어있는 '${saju?.lacking || '수(물)'}' 기운을 더욱 메마르게 억누르고 뇌파를 지글지글 과열시키는 '상극(相剋)의 흉물 노이즈'들입니다. 원국과 충돌하는 이질적인 소품이나 산만한 시각적 독소들이 데스크 주변을 장악하는 순간, 시신경을 통해 스트레스 호르몬인 코르티솔이 과다 분비되며 극심한 무기력증과 깊은 블랙아웃 슬럼프에 직면하게 됩니다.`;
    ...
    p2 = `치열한 스펙 경쟁이 펼쳐지는 커리어 전장에서 당신이 경쟁자들을 완벽하게 씹어먹으며 돋보일 수 있는 핵심 동력은 원국 내 가장 폭발적인 '${saju?.excessive || '목(나무)'}' 기운에 있습니다. ${excessObj.power || ''} 이 주특기 에너지를 100% 가동할 수 있는 무대에 서는 순간, 남들이 수십 년의 짬바를 채워야 도달할 수 있는 전문성의 경지를 단숨에 관통해 내는 압도적인 퍼포먼스를 과시하게 됩니다.\n\n그러나 커리어의 수직 상승을 가로막고 깊은 늪으로 끌어내리는 가장 무서운 함정은 바로 '${saju?.lacking || '수(물)'}' 기운이 결핍된 직무를 억지로 연기하며 수행할 때 발생합니다. ${lackObj.pain || ''} 원국과 소통되지 않는 상극의 직군에 갇혀 억지로 전두엽을 쥐어짜면, 결국 본인의 장점은 퇴색되고 예민함과 독선적인 단점만이 수면 위로 부각되어 깊은 번아웃과 잦은 이직이라는 참혹한 커리어 슬럼프에 직면하게 됩니다.`;
    title3 = "🗝️ [VVIP 프라이빗 시크릿 솔루션] 독점적 몸값 쟁취를 위한 3대 커리어 프로토콜";
    p3 = `【 STEP 1. 평균의 함정을 비웃는 '대체불가 특수 틈새 독점술' 】\n남들이 부러워하는 대기업의 안정적인 부품 포지션이나 뻔한 전문직 타이틀에 만족하는 얄팍한 목표를 당장 소각하십시오. 오직 ${name}님 본인만이 뿜어낼 수 있는 광기 어린 세계관으로 시장의 룰을 새로 정의할 [ ${ELEMENT_PRESCRIPTION[saju?.lacking || '수(물)']?.job || '특수 전문 융합 분야'} ]로의 진출을 강력히 제안합니다. 임계점을 돌파하는 순간 몸값이 천정부지로 폭등합니다.\n\n【 STEP 2. 약점을 흉기로 치환하는 '외과 수술적 T자형 인재 전략' 】\n선천적으로 타고나 가장 쉽고 비상하게 회전하는 주특기 기운(세로축)을 세계 최고 수준으로 날카롭게 연마함과 동시에, 평소 쳐다보기도 두려워했던 결핍 오행 '${saju?.lacking || '수(물)'}'의 비즈니스 지식(가로축)을 처절하게 학습하여 뇌에 강제 융합하십시오. 이 이질적인 두 에너지의 교차점에 선 융합형 몬스터 인재는 시장에서 부르는 게 값이 됩니다.\n\n【 STEP 3. 대운 변곡점 기반의 '결정적 승부수 타이밍 설계' 】\n사주 명리학에서 커리어의 판도가 수직으로 점프하는 거대한 대운의 변곡점이 도래하기 전까지는 섣불리 조직을 탈출하거나 멘탈을 소모하지 마십시오. 현재의 포지션에서 칼을 갈며 나만의 정밀한 데이터베이스와 인맥을 조용히 흡수하다가, 운기의 물길이 완벽하게 열리는 골든 타이밍에 묵직한 출사표를 던져 판을 장악하셔야 합니다.`;
    ...
    p2 = `현재 대인관계나 비즈니스 협상 테이블에서 타인의 논리를 무장 해제시키고 판을 주도하는 압도적인 장악력은 넘쳐흐르는 주 기운에서 기인합니다. 상대방은 뚜렷한 논리적 근거가 없음에도 불구하고 당신의 확에 찬 눈빛과 서늘한 여유에 매료되어 맹목적으로 고개를 끄덕이게 됩니다. 회의실이나 사교 모임의 주도권이 언제나 당신의 손아귀로 빨려 들어오는 경이로운 중력의 원천입니다.\n\n그러나 완벽해 보이는 이 통치 리더십을 한순간에 붕괴시키고 주변 사람들을 떠나보내 '고독한 폭군'으로 전락시키는 치명적인 아킬레스건은 바로 '${saju?.lacking || '수(물)'}' 기운의 극심한 결핍에 있습니다. 타인의 감정적 주파수에 공명하고 교감하는 생체 소통 에너지가 고갈되는 순간, 당신의 서늘한 카리스마는 피도 눈물도 없는 '독선과 아집'으로 주변에 왜곡되어 전달되며 참혹한 관계의 단절과 배신을 겪게 됩니다.`;
...
export default function App() {
  ...
  useEffect(() => {
    setTimeout(() => { try { window.scrollTo({ top: 0, left: 0, behavior: 'auto' }); } catch(e){} }, 50);
  ...
  // 🔥 1번 수술: 존재하지 않는 newSearchParams를 정석 문법(new URLSearchParams)으로 영구 수술! 🔥
  useEffect(() => {
    try {
