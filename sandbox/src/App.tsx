...
const generateProfessionalReport = (user, saju, menuId) => {
  ...
  if (menuId === 1) {
    analysis1Text = [
      `본 프라이빗 컨설팅은 시중의 가벼운 사주 풀이처럼 막연한 위로나 근거 없는 칭찬을 나열하는 행위를 엄격히 지양합니다. 인간이 태어난 년, 월, 일, 시의 우주적 좌표를 천문학적 황경 기준으로 심층 해부한다는 것은, 당신의 뇌와 무의식이 세상의 방대한 지식과 정보를 받아들이고 처리할 때 사용하는 가장 근본적인 인지 필터 알고리즘을 찾아내는 일입니다. 정밀 해독 결과, ${name}님의 일간(Day Master)은 [ ${dm.name} ]의 기운으로 굳건하게 세팅되어 있습니다.`,
    ...
    if (age <= 7) {
      ...
      summaryText = "비싼 영어 유치원과 조기 교육 교재보다 천 배, 만 배 중요한 것은, 내 아이의 사주 원국에 완벽하게 들어맞는 '스스로 탐구하고 표현하는 70%의 놀이 비율'을 그 어떤 주변의 불안감과 맘카페의 소음에도 흔들림 없이 묵묵히 지켜내는 부모의 서늘하고 묵직한 뚝심입니다.";
      conclusionText = [
      ...
      summaryText = "초등 시기의 무리한 학원 선행 학습보다 천 배, 만 배 중요한 것은, 내 아이의 사주 원국에 완벽하게 들어맞는 '설명 중심의 능동적 출력 습관'을 그 어떤 유혹과 귀찮음에도 흔들림 없이 매일 묵묵히 지켜내어 중고등 6년의 강철 같은 뼈대를 세워주는 것입니다.";
      conclusionText = [
      ...
      summaryText = "수백만 원짜리 일타 강사의 비법 자료보다 천 배, 만 배 중요한 것은, 내 사주 원국에 완벽하게 들어맞는 '입력 30% : 출력 70%의 잔혹한 황금비율'을 수능장 들어가는 그날까지 어떤 유혹에도 흔들림 없이 묵묵히 지켜내는 독하고 서늘한 뚝심입니다.";
      conclusionText = [
      ...
      prescriptionText = [
        ...
        `【 STEP 1. 성인 뇌 가소성을 역해킹하는 딥워크(Deep-Work) 진입 의식 】\n성인의 뇌는 10대처럼 말랑말랑하지 않으며, 일상의 수많은 잡념(돈, 관계, 미래에 대한 불안 등)으로 꽉 차 있습니다. 책상에 앉자마자 공부를 시작하는 것은 불가능합니다. 본격적인 지식 입력 전 최소 15분 동안은, 당신의 사주에 메말라 있는 '${saju.lacking}'의 에너지를 뇌에 직접적으로 꽂아 넣는 [ ${lackProp.action.split(",")[0]} ]의 시간을 강제로 확보하셔야 합니다. 철저하게 휴대폰을 차단하고 15분간 가벼운 명상, 업무 리스트 시각화, 혹은 가장 쉬운 기출문제 3개 풀기 등으로 뇌파를 강제로 알파파 상태로 예열시킨 후 딥워크에 진입하십시오.`,
      ...
      summaryText = "어른의 공부에서 수백 페이지의 두꺼운 기본서 정독보다 천 배, 만 배 중요한 것은, 내 사주 원국에 완벽하게 들어맞는 '입력 20% : 실전 출력 80%의 잔혹한 역발상 황금비율'을 그 어떤 완벽주의의 유혹에도 흔들림 없이 냉혹하게 밀어붙이는 뚝심입니다.";
      conclusionText = [
    ...
    summaryText = "상위 0.1%의 진정한 성공은 남들이 다 피 터지게 매달리는 뻔한 토익 점수나 자격증 1, 2점 경쟁에서 절대 오지 않습니다. 내 사주의 압도적이고 폭력적인 강점과, 나를 가장 괴롭히는 뼈아픈 결핍이 내면에서 격렬하게 융합하여 만들어내는 '대체 불가능한 가치' 그 자체에 세상의 모든 부와 명예가 거대한 자석처럼 쏟아지게 됩니다.";
    conclusionText = [
...
export default function SajuLearningApp() {
  ...
  // 🔥 1. 화면 밖 숨겨진 요소들을 캡처해 PDF 파일로 즉시 다운로드 해주는 마법 함수 🔥
  const downloadDirectPDF = async () => {
    if (!userSaju || !userInfo) return;
    
    // 버튼을 누르면 상태가 로딩중으로 바뀜
    ...
    setTimeout(async () => {
      try {
        ...
        // 인쇄창 띄우지 않고, 화면을 캡처해서 바로 PDF 파일로 저장!
  ...
  return (
    <div className="min-h-screen text-[rgba(255,255,255,0.88)] font-sans relative bg-[#0D0B1A]">
            ...
            {unlockedMenus.includes(selectedMenu.id) && (
              <div className="bg-[#111625] rounded-[20px] shadow-xl p-5 mb-6 relative overflow-hidden border border-[#E8C87A]/30">
                ...
                <div className="relative z-10 text-center">
                  ...
                  <button onClick={downloadDirectPDF} disabled={isDownloading} className="inline-flex items-center gap-2 bg-[linear-gradient(135deg,#D4A843,#E8C050)] text-[#1A1530] font-black text-[13px] px-5 py-4 rounded-xl shadow-[0_4px_15px_rgba(212,168,67,0.3)] hover:scale-[1.02] disabled:opacity-50 disabled:scale-100 transition-transform w-full justify-center">
                    <Download size={18} strokeWidth={2.5} />
                    {isDownloading ? "PDF 생성 중... 잠시만 기다려주세요" : "10,000자급 디자인 리포트 저장 (PDF)"}
                  </button>
                </div>
              </div>
            )}
      ...
      {currentView === 'result' && unlockedMenus.includes(selectedMenu.id) && (
        <div id="pdf-report-container" className="absolute top-0 left-[-9999px] w-[800px] bg-[#FDFBF7] text-[#111625] font-serif z-[-10]">
          ...
          <div className="px-12 pb-12 bg-[#FDFBF7]">
            {generateProfessionalReport(userInfo, userSaju, selectedMenu.id).map((section, idx) => (
              <div key={idx} className="mb-14" style={{ pageBreakInside: 'avoid' }}>
            ...
            {/* 법적 고지사항 영역 */}
            <div className="mt-20 pt-10 border-t-[3px] border-[#D4A843] text-[12px] text-[#777d8a] leading-relaxed">
